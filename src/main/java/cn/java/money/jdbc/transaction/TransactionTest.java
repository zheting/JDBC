 ...
 * 4.哪些操作会导致数据的自动提交？
 * 		>DDL操作一旦执行，都会自动提交。
 * 			>set autocommit = false 对DDL操作失效（不起作用）
...
public class TransactionTest {
	...
	public void testUpdate(){
		...
		update(sql1, "AA"); //每次都是获取一个新的Connection,执行完sql以后关闭
		
		//模拟网络异常， AA的100减去了，BB的100没有加上去，且AA减掉的100也没有回滚。
	...
	public int update(Connection conn,String sql, Object... args) {// sql中占位符的个数与可变形参的长度相同！
		...
		try {
			...
			// 4.资源的关闭,这里只关闭了Statement，没有关闭Connection
	...
	public void testTransactionSelect() throws Exception{
		...
		//获取当前连接的隔离级别  int TRANSACTION_REPEATABLE_READ  = 4;
